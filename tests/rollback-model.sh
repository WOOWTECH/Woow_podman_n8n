...
SHIMS=$HERE/shims
FILTER=${1:-}
ROOT=$(mktemp -d "${TMPDIR:-/tmp}/n8n-rollback-tests.XXXXXX")
...
mk_legacy() {
  local name=$1 policy=$2 image=docker.io/n8nio/n8n:2.38.7 d
  ...
  printf 'imgid-n8n' >"$d/image_id"
  printf 'imgid-n8n' >"$SHIM_STATE/image-ids/${image//[\/:@]/_}"
  printf 'bridge' >"$d/netmode"
  printf 'false' >"$d/autoremove"
  printf 'n8n' >"$d/project"
  printf '%s' "$name" >"$d/service"
  printf '4096' >"$d/sizerw"
  printf 'volume|n8n_n8n_data|/vol/n8n_n8n_data|/home/node/.n8n|true|rprivate\n' >"$d/mounts"
  printf 'n8n-network|%s cid-%s |10.89.0.7|aa:bb:cc:dd:ee:01\n' "$name" "$name" >"$d/networks"
  printf '5678/tcp|127.0.0.1:5678 \n' >"$d/ports"
  printf 'io.podman.compose.project=n8n\n' >"$d/labels"
  : >"$d/label"
  printf '%s\0' /usr/bin/podman run "--name=$name" -d --rm --replace \
    --cidfile "/run/user/1000/$name.cid" --label io.podman.compose.project=n8n \
    -v n8n_n8n_data:/home/node/.n8n --net n8n-network -e N8N_PORT=5678 \
    n8nio/n8n:2.38.7 n8n start >"$d/createcommand.argv0"
}
...
t_disabled_restart_unit_keeps_the_rename_path() {
  mk_legacy n8n-postgres unless-stopped
  mk_legacy n8n unless-stopped
  eq "$(ql_rollback_strategy n8n-postgres n8n 2>/dev/null)" rename "strategy on a toypark-like host"
  expect_ok app_legacy_retire rename 20260914 "$T/bk" n8n-postgres n8n
  has "$OUT" "renamed n8n -> n8n-legacy-20260914"
  eq "$(ncalls 'podman rename n8n-postgres n8n-postgres-legacy-20260914')" 1 "rename of the db"
  eq "$(ncalls 'podman rename n8n n8n-legacy-20260914')" 1 "rename of n8n"
  ...
  podman container exists n8n-legacy-20260914 || die_t "the renamed container is missing"
  # and the rollback renames them straight back
  expect_ok app_legacy_restore 20260914 "$T/bk" n8n-postgres n8n
  has "$OUT" "renamed n8n-legacy-20260914 -> n8n"
  podman container exists n8n || die_t "the rollback did not bring n8n back"
  eq "$(ncalls 'podman create')" 0 "a renamed container is not recreated"
}

t_always_policy_with_a_disabled_unit_is_still_rename() {
  mk_legacy n8n always
  mk_legacy n8n-postgres always
  eq "$(ql_rollback_strategy n8n-postgres n8n 2>/dev/null)" rename "a disabled unit never revives anything"
}
...
t_enabled_restart_unit_and_always_policy_takes_the_capture_path() {
  ...
  mk_legacy n8n-postgres always
  mk_legacy n8n always
  eq "$(ql_rollback_strategy n8n-postgres n8n 2>/dev/null)" capture "strategy on an openclaw-like host"
  expect_ok app_legacy_capture "$T/bk" n8n-postgres n8n
  for c in n8n n8n-postgres; do
  ...
  expect_ok app_legacy_retire capture 20260914 "$T/bk" n8n-postgres n8n
  has "$OUT" "removed n8n;"
  eq "$(ncalls 'podman rename')" 0 "the capture path must not rename"
  podman container exists n8n && die_t "n8n was not removed"
  podman container exists n8n-legacy-20260914 && die_t "the capture path must not leave a renamed copy"
  return 0
}
...
t_the_capture_path_never_removes_the_anonymous_volumes() {
  enable_restart_unit
  mk_legacy n8n always
  expect_ok app_legacy_capture "$T/bk" n8n
  expect_ok app_legacy_retire capture 20260914 "$T/bk" n8n
...
t_the_rollback_recreates_a_captured_container_with_its_policy() {
  ...
  expect_ok app_legacy_capture "$T/bk" n8n
  expect_ok app_legacy_retire capture 20260914 "$T/bk" n8n
  expect_ok app_legacy_restore 20260914 "$T/bk" n8n
  has "$OUT" "recreated n8n"
  podman container exists n8n || die_t "the rollback did not recreate n8n"
  eq "$(ql_container_restart_policy n8n)" always "the original restart policy comes back"
}

t_capture_refuses_a_container_the_library_cannot_replay() {
  enable_restart_unit
  mk_api_created n8n always
  expect_fail app_legacy_capture "$T/bk" n8n
  has "$OUT" "podman API"
...
t_retire_refuses_to_remove_without_a_capture() {
  enable_restart_unit
  mk_legacy n8n always
  expect_fail app_legacy_retire capture 20260914 "$T/bk" n8n
  has "$OUT" "no rollback copy of n8n"
...
t_capture_is_idempotent_between_prepare_only_and_the_cutover() {
  enable_restart_unit
  mk_legacy n8n always
  expect_ok app_legacy_capture "$T/bk" n8n # --prepare-only
  expect_ok app_legacy_capture "$T/bk" n8n # the cutover reuses the same backup dir
  has "$OUT" "already in"
}
...
t_the_capture_path_works_with_an_empty_suffix() {
  ...
  mk_legacy n8n always
  expect_ok app_legacy_capture "$T/bk" n8n
  expect_ok app_legacy_retire capture "" "$T/bk" n8n
  podman container exists n8n && die_t "n8n was not removed"
  expect_ok app_legacy_restore "" "$T/bk" n8n
  has "$OUT" "recreated n8n"
  eq "$(ql_container_restart_policy n8n)" always "the original restart policy comes back"
  # and with no capture either, the refusal names only what could exist
  expect_fail app_legacy_restore "" "$T/empty" n8n
